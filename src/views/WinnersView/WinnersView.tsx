...
export const WinnersView: FC = React.memo(function WinnersView() {
  ...
  return (
    <Stack direction={"column"} alignItems={"stretch"} spacing={0}>
      <div style={{ padding: "16px", color: "#e8e8e8", fontSize: 20, fontWeight: 700, borderBottom: "2px solid #2a2a35", background: "#16161e" }}>
        <Stack direction={"row"} alignItems={"center"} spacing={8}>
          <Icon name="trophy" size={20} color="#fbbf24" />
          <span>Winners ({totalCount})</span>
        </Stack>
      </div>
      ...
      {winners.length === 0 ? (
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", color: "#e8e8e8", fontSize: 14 }}>
            <thead>
              <tr style={{ background: "#16161e", borderBottom: "2px solid #2a2a35" }}>
                <th style={thStyle}>№</th>
                <th style={thStyle}>Car</th>
                <th style={thStyle}>Name</th>
                <th
                  style={{ ...thStyle, cursor: "pointer", color: sortField === "wins" ? "#6c63ff" : "#e8e8e8" }}
                  onClick={() => handleSort("wins")}
                >
                  Wins{sortIndicator("wins")}
                </th>
                <th
                  style={{ ...thStyle, cursor: "pointer", color: sortField === "time" ? "#6c63ff" : "#e8e8e8" }}
              ...
              {winners.map((winner, index) => (
                <tr
                  ...
                  style={{
                    ...
                    background: index % 2 === 0 ? "#16161e" : "#0f0f13",
                  }}
                >
